...
namespace FunctionalTests.Search
{
    ...
    public class SearchHelper
    {
        public static void CreateSampleIndex(ISearchProvider provider, string scope)
        {
            ...
            provider.Index(scope, "catalogitem", CreateDocument("sad121", "red shirt", "red", 10, 3));
        ...
        private static ResultDocument CreateDocument(string key, string name, string color, decimal price, int size)
        {
            ...
            doc.Add(new DocumentField("price_usd_default_value", LuceneQueryHelper.ConvertDecimalToSortable(price), new[] { IndexStore.YES, IndexType.NOT_ANALYZED }));
            ...
            doc.Add(new DocumentField("size", LuceneQueryHelper.ConvertToSearchable(size), new[] { IndexStore.YES, IndexType.NOT_ANALYZED }));
