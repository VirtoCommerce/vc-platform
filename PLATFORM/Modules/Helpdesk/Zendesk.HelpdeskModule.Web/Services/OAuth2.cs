...
namespace Zendesk.HelpdeskModule.Web.Services
{
    ...
    public abstract class OAuth2
    {
        ...
        {
            var url = getVariable("services_uri") ?? getVariable("base_uri");
        ...
        public string getAccessTokenFromAuthorizationCode(string code)
        {
            ...
            { "client_secret", _variables["client_secret"] }, {"code", code}, {"redirect_uri", _variables["redirect_uri"]}, {"scope", _variables["scope"]} };
        ...
        {
            var cookie = new HttpCookie("oauth2_" + getVariable("client_id")) { Value = getQueryString(session) };
        ...
        protected Dictionary<string, string> getSessionObject(AccessToken a)
        {
            ...
            if (a != null)
            {
                ...
                session.Add("bearer", a.bearer);
                session.Add("scope", a.scope);
