	...
	function recalculateTotals(operation)
	{
		if (angular.isDefined(operation.operationType)) {
			if (operation.operationType.toLowerCase() == 'customerorder') {
				...
				operation._itemsDiscountTotal = _.reduce(operation.items, function (memo, x) { return memo + parseFloat(x.discountAmount) }, 0);
				operation._discountTotal += operation._itemsDiscountTotal;
				operation._discountTotal += _.reduce(operation.shipments, function (memo, x) { return memo + parseFloat(x.discountAmount) }, 0);

				operation._taxTotal = 0;
				operation._itemsTaxTotal = _.reduce(operation.items, function (memo, x) { return memo + parseFloat(x.tax); }, 0);
				operation._taxTotal += operation._itemsTaxTotal;
				operation._taxTotal += _.reduce(operation.shipments, function (memo, x) { return memo + parseFloat(x.tax); }, 0);

				operation._itemsTotal = 0;
				_.each(operation.items, function (x) {
					x._total = parseInt(x.quantity) * (parseFloat(x.price) || 0) + (parseFloat(x.tax) || 0);
					x._total = Math.round(x._total * 100) / 100;
					operation._itemsTotal += x._total;
				});
				...
				operation._subTotal = parseFloat(_.reduce(operation.items, function (memo, x) { return memo + x.price * x.quantity; }, 0) || 0);
				operation._taxTotal = parseFloat(operation.tax) || 0;
				operation._shippingTotal = parseFloat(operation.sum) || 0;
