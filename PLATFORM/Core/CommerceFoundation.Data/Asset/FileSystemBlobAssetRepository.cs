...
namespace VirtoCommerce.Foundation.Data.Asset
{
    ...
    public class FileSystemBlobAssetRepository : IAssetRepository, IBlobStorageProvider, IUnitOfWork, IAssetUrlResolver
    {
        ...
        public FileSystemBlobAssetRepository(string storagePath, IAssetEntityFactory assetEntityFactory)
            : this(storagePath, null, assetEntityFactory)
        {
        }

        public FileSystemBlobAssetRepository(string storagePath, string publicUrl, IAssetEntityFactory assetEntityFactory)
        {
        ...
        public virtual string Upload(UploadStreamInfo info)
        {
            ...
            return GetAbsoluteUrl(info.FileName);
        }
        ...
        {
            var result = string.Concat(_publicUrl, assetId);
            return result;
        }
        ...
        public string GetRelativeUrl(string absoluteUrl)
        {
            ...
            if (_publicUrl != null && absoluteUrl != null && absoluteUrl.StartsWith(_publicUrl, StringComparison.OrdinalIgnoreCase))
            {
                result = absoluteUrl.Substring(_publicUrl.Length);
            }
