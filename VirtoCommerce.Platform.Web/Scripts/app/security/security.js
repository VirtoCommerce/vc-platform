angular.module('platformWebApp')
    .config(['$stateProvider', '$httpProvider', function ($stateProvider, $httpProvider) {
        ...
        $stateProvider.state('changePasswordDialog',
            {
                url: '/changepassword',
                templateUrl: '$(Platform)/Scripts/app/security/login/changePassword.tpl.html',
                params: {
                    userName: null
                },
                controller: ['$scope', '$stateParams', 'platformWebApp.accounts', '$state', function ($scope, $stateParams, accounts, $state) {
                    ...
                    $scope.ok = function () {
                        ...
                        accounts.changepassword({ id: $scope.userName }, postData, function(data) {
                            if (data.errors.length > 0) {
                                $scope.saveError = data.errors[0];
                            } else {
                                $state.go("workspace");
                            }
                        });
        ...
        $stateProvider.state('changeApiSecretKey',
            {
                ...
                templateUrl: '$(Platform)/Scripts/app/security/login/changeApiSecretKey.tpl.html',
                params: {
                    apiAccount: null,
                    userName: null
                },
                controller: ['$scope', '$stateParams', 'platformWebApp.accounts', '$state', function ($scope, $stateParams, accounts, $state) {
                    var apiAccount = $stateParams.apiAccount;
                    var newApiAccount = {};
                    if (apiAccount) {
                        $scope.userName = $stateParams.userName;
                        $scope.appId = apiAccount.appId;
                        $scope.secretKey = apiAccount.secretKey;

                        $scope.generate = function() {
                            accounts.generateNewApiKey({}, apiAccount, function (data) {
                                newApiAccount = data;
                                $scope.secretKey = newApiAccount.secretKey;
                            });
                        }

                        $scope.save = function() {
                            accounts.get({ id: $scope.userName }).$promise.then(function (user) {

                                for (var i = 0; i < user.apiAccounts.length; i++) {
                                    if (user.apiAccounts[i].id === newApiAccount.id) {
                                        user.apiAccounts[i] = newApiAccount;
                                    }
                                    break;
                                }

                                accounts.update({}, user, function(data) {
                                    $state.go("workspace");
                                });
                            });
                        }
