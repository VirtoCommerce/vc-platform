...
{
	[Serializable]
	public class ConditionGeoIspTopLevel : TypedExpressionElementBase, IExpressionAdaptor
	{
		private MatchContainsStringElement _valEl;

		public ConditionGeoIspTopLevel(IExpressionViewModel expressionViewModel)
			: base("Are browsing from an ISP with top domain level []".Localize(), expressionViewModel)
		{
			WithLabel("Browsing from an ISP with top domain level ".Localize());
		...
		public System.Linq.Expressions.Expression<Func<IEvaluationContext, bool>> GetExpression()
		{
			...
			var propertyValue = System.Linq.Expressions.Expression.Property(castOp, typeof(PriceListAssignmentEvaluationContext).GetProperty("GeoIspTopLevel"));
