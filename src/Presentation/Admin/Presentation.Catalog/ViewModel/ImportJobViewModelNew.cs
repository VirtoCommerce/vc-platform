...
{
    public class ImportJobViewModelNew : WizardStepViewModelBase, IImportJobViewModel, IClosable, ISupportAcceptChanges, IMinimizable, IOpenTracking, ISupportDelayInitialization
    {
        private ImportJob _originalItem;
        private ICatalogEntityFactory _entityFactory;
        private IImportRepository _itemRepository;
		private IColumnMappingEditViewModel _mappingEditVM;
		private IImportService _importService;
		private IPickAssetViewModel _pickAssetVM;
		private IColumnMappingViewModel _columnMappingVM;
		...
		private void RaiseFilePickInteractionRequest()
		{
			...
			CommonConfirmRequest.Raise(
				new ConditionalConfirmation(_pickAssetVM.Validate) { Content = _pickAssetVM, Title = "Select file" },
				(x) =>
				{
					if (x.Confirmed)
					{
		...
		private void RaiseCreateMappingInteractionRequest()
		{
			...
			CommonConfirmRequest.Raise(
				new ConditionalConfirmation() { Content = _mappingEditVM, Title = "Create mapping" },
				(x) =>
				{
					if (x.Confirmed)
					{
		...
		private void RaiseItemEditInteractionRequest(MappingItem originalItem)
		{
			...
			var param = new ColumnMappingEntity { MappingItem = item, CsvColumnsList = CsvFileColumns };
		...
		public EntityImporterBase SelectedEntityImporter
		{
			get
			{
