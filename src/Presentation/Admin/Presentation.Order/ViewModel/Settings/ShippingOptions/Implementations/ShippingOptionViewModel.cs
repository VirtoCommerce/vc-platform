...
namespace VirtoCommerce.ManagementClient.Order.ViewModel.Settings.ShippingOptions.Implementations
{
	public class ShippingOptionViewModel : ViewModelDetailAndWizardBase<ShippingOption>, IShippingOptionViewModel
	{
		...
		{
			ViewTitle = new ViewTitleBase { SubTitle = "SETTINGS", Title = "Shipping Option" };
		...
		protected override RefusedConfirmation CancelConfirm()
		{
			...
			{
				Content = "Save changes to Shipping option '" + DisplayName + "'?",
		...
		{
			get { return "Enter Shipping Option information."; }
		}
		...
		private void RemoveShippingPackage(ShippingPackage selectedItem)
		{
			...
			var confirmation = new ConditionalConfirmation
				{
					Title = "Remove shipping package confirmation",
					Content = string.Format("Remove shipping package '{0}?", itemToRemove.ShippingOptionPackaging)
				};
		...
		private void InitializeGatewayValuesForSave()
		{
			...
			{
				var item = InnerItem.ShippingGatewayPropertyValues.FirstOrDefault(x => x.Name == currentItem.InnerItem.Id);
				if (item == null)
				{
