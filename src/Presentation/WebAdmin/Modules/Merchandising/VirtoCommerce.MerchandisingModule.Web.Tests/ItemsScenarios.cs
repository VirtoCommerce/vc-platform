...
namespace VirtoCommerce.MerchandisingModule.Web.Tests
{
    ...
    public class ItemsScenarios
    {
        [Fact]
        public void Can_create_new_product()
        {
            var azureRepository = new AzureBlobAssetRepository(new AssetEntityFactory());
            var assetService = new AssetService(
                azureRepository,
                azureRepository);

            var service = new MerchantItemService(new EFCatalogRepository("VirtoCommerce"), assetService);
            var code = Guid.NewGuid().ToString();
            var product = new Product() {Id = code, Code = code, Name = "Sample Test Product", Catalog = "Samsung"};
            product.Properties = new PropertyDictionary();
            ...
            var controller = new MerchantItemsController(service);
            controller.Request = new HttpRequestMessage();
            controller.Request.SetConfiguration(new HttpConfiguration());
            var result = controller.CreateProduct(categoryId, product);

            var product2 = controller.GetProduct(product.Id);
            Assert.True(product.Id == product2.Id);
            Assert.True(product.Variations.Count() == 1);

            // Now check the status
            object status = null;
            product2.Properties.TryGetValue("PublishStatus", out status);

            Assert.True(status.ToString() == "Draft");

            // Now publish the item by chaning document status
            var updateDelta = new Delta<Product>(typeof(Product));
            var deltaAttributes = new PropertyDictionary();
            deltaAttributes.Add("PublishStatus", "Published");
            updateDelta.TrySetPropertyValue("Properties", deltaAttributes);
            updateDelta.TrySetPropertyValue("Id", product2.Id);
            controller.PatchProduct("caetgory", updateDelta);

            product2 = controller.GetProduct(product.Id);
            product2.Properties.TryGetValue("PublishStatus", out status);

            Assert.True(status.ToString() == "Published");

            // now remove the product
            controller.DeleteProduct(product2.Id);
