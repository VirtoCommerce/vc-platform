...
namespace VirtoCommerce.Framework.Core.Caching
{
	...
	public class CacheManager
	{
		...
		public void Remove(CacheKey cacheKey)
		{
			...
			var provider = GetProvider(cacheKey.CacheGroup);
			if (provider != null)
			{
				provider.RemoveItem(cacheKey);
			}
		}
		...
		public void Put(CacheKey cacheKey, object cacheValue)
		{
			...
			if (repository != null)
			{
				...
				repository.PutItem(cacheKey, cachedObj, expirationTimeout);
			}
		}
		...
		public T Get<T>(CacheKey cacheKey)
		{
			...
			if (repository != null)
			{
				var cachedObj = repository.GetItem(cacheKey);
				if (cachedObj != null)
				{
		...
		public T GetSynchronized<T>(CacheKey cacheKey, Func<T> getValueFunction, object syncObject)
		{
			if (cacheKey == null)
			{
				throw new ArgumentNullException("cacheKey");
			...
			else
			{
				...
				if (cachedObj == null)
				{
					lock (syncObject)
					{
						cachedObj = repository.GetItem(cacheKey);
						if (cachedObj == null)
						{
							cachedObj = new CachedObject(getValueFunction());
