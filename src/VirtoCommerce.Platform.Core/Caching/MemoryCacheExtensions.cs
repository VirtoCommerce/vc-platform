...
namespace VirtoCommerce.Platform.Core.Caching
{
    public static class MemoryCacheExtensions
    {
            ...
            where TItem : IEntity
        {
            ...
            {
                using (await AsyncLock.GetLockByKey(keyPrefix).GetReleaserAsync().ConfigureAwait(false))
                {
        ...
        public static async Task<TItem> GetOrCreateExclusiveAsync<TItem>(this IMemoryCache cache, string key, Func<MemoryCacheEntryOptions, Task<TItem>> factory, bool cacheNullValue = true)
        {
            ...
            {
                using (await AsyncLock.GetLockByKey(key).GetReleaserAsync().ConfigureAwait(false))
                {
