...
namespace VirtoCommerce.Platform.Data.Modularity
{
    ...
    //    public ModuleInstallerImpl(IModuleCatalog moduleCatalog, IModuleManifestProvider manifestProvider, string installedPackagesPath, string[] updateServersUrls, ICacheManager<object> cacheManager)
    //    {
    ...
    //    public ManifestModuleInfo LoadModule(string url)
    //    {
    //        var fullPath = Path.GetFullPath(url);
    //        var manifest = ReadPackageManifest(fullPath);
    //        var result = new ManifestModuleInfo(manifest);
    //        return result;
    //    }
     
    //    public IEnumerable<ManifestModuleInfo> GetAllModules()
    //    {
    ...
    //            .Select(m => new ManifestModuleInfo(m))
    //            .OrderBy(m => m.Title)
    //            .ToArray();
    ...
    //    public void Install(ManifestModuleInfo moduleInfo, IProgress<ProgressMessage> progress)
    //    {
    //        Report(progress, ProgressMessageLevel.Info, "Installing '{0}'.", moduleInfo.FullPhysicalPath);

    //        var sourcePackage = LoadModule(moduleInfo.FullPhysicalPath ?? moduleInfo.PackageUrl);
