...
5. References to NuGet packages:
   1. **CustomerReviews.Core**: add reference to **_VirtoCommerce.Platform.Core_** (latest version);
   1. **CustomerReviews.Data**: add reference to **_VirtoCommerce.Platform.Data_** (latest version), **_Microsoft.EntityFrameworkCore.Tools_** (same version as Microsoft.EntityFrameworkCore is used in VC Platform, 3.1.2 as of moment of writing);
...
3. If any model-related changing/changed events were defined in **Events** folder, ensure that each of them derive from the base `GenericChangedEntryEvent<T>` class.
...
1. **Repositories** folder
    1. Create **CustomerReviewsDbContext.cs**
        ...
        * Override _OnModelCreating_ method, fill its content by moving the code from _CustomerReviewRepository.OnModelCreating_. Refactoring the entity mapping and setting max length to property:
            ```cs
    ...
    3. Update **ICustomerReviewsRepository.cs**
        * If the module is an extension, then derive from derived module's repository interface.
        * Refactor all methods to **be asynchronous**: return `Task<>`
        * Rename all methods to have suffix `Async`
    3. Update **CustomerReviewsRepository.cs**
        * Refactor **_CustomerReviewsRepository_** class to derive from `DbContextRepositoryBase<CustomerReviewsDbContext>` or if the module is an extension then derive from the derived module's repository.
        * Refactor the constructors to leave only one, taking the only  **_CustomerReviewsDbContext_** parameter:
            ...
            ```
        * Refactor **_CustomerReviews_** property to access data using DbSet:
            ```cs
...
2. **Caching** folder
    ...
    2.  Add new class **_CustomerReviewCacheRegion_** and derive it from generic `CancellableCacheRegion<T>` class:
        ```cs
...
2. **_Handlers_** folder. All event handlers (if any) should be defined in / moved to this folder.
