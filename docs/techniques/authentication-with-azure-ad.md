...
If you use Azure AD authentication and want to allow users from any tenant to connect to your Virto Commerce application,
...
4. In cases when **upn** claim is not being passed the system can be configured to use **preferred_username** claim instead using the following setting:
```json
	{
		"UsePreferredUsername": true
	}
```
5. In cases when **upn** claim is not being passed the system can be configured to use **email** claim instead using the following setting:
```json
	{
		"UseEmail": true
	}
