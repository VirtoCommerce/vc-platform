...
Each payment method has to be implemented as a separate module. To learn how to create new modules see the following [link](../working-with-platform-manager/extending-functionality/creating-new-module.md).

To add a new payment method you need to implement an abstract class PaymentMethod. 
...
* has to impelement querystring request processing. The request comes to **push url** from outside payment system or frontend. The **push url** is set in account settings of most payment systems or during payment transaction processing in outside payment system (ProcessPayment method).
...
**Important**: In order to set url for push notifications as well as other required url links use **Secure Url** setting of store. In case of the setting returns null use **Store Url** setting. The system uses the only route for payment system requests - **https://{your-secure-url}/admin/api/paymentcallback**
...
After the payment method is implemented, you have to define settings of the payment method in the module manifest. To learn how to work with the manifest file see the following [link](docs/vc2devguide/working-with-platform-manager/extending-functionality/managing-module-settings).
...
Next, create the module package of the payment method. To learn how to create module packages see the following [link](../development-scenarios/creating-module-packages.md).

Finally, install the created module package in VirtoCommerce platform manager. To learn how to install module packages see [link](../../user-guide/configuration/modules-management.md).

The payment method module should become available in the VirtoCommerce system. 

## Working with the payment method

As a result of registering a new payment method module in the platform it becomes available to select in the store's payment methods settings. On the screenshot below the "Payment methods" the widget has 0 value. That means that none of the registered payment methods are assigned to the store and won't be available to select in the storefront. 
...
The functionality of the user interface can be extended by adding widgets. To learn how to add widgets see [link](../working-with-platform-manager/basic-functions/widgets.md).

![](../../assets/images/docs/base643b586b50db494e93.png) 

When the check mark appears, the payment method activation is available in the storefront. 
