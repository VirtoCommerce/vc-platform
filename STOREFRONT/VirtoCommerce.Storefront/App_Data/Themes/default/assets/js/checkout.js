...
storefrontApp.controller('checkoutController', ['$rootScope', '$scope', '$window', 'customerService', 'cartService', function ($rootScope, $scope, $window, customerService, cartService) {
    ...
    $scope.setCountry = function (addressType, countryName) {
        ...
        if (addressType === shippingAddressType) {
            $scope.checkout.selectedShippingCountry = country;
            $scope.checkout.shipment.deliveryAddress.countryCode = country.code3;
            $scope.checkout.shipment.deliveryAddress.regionId = null;
            $scope.checkout.shipment.deliveryAddress.regionName = null;
            if (country.code3) {
                getCountryRegions(shippingAddressType, country.code3);
            }
        }
        if (addressType === billingAddressType) {
            $scope.checkout.selectedBillingCountry = country;
            $scope.checkout.payment.billingAddress.countryCode = country.code3;
            $scope.checkout.payment.billingAddress.regionId = null;
            $scope.checkout.payment.billingAddress.regionName = null;
            if (country.code3) {
                getCountryRegions(billingAddressType, country.code3);
            }
        }
    }
    ...
    $scope.setCountryRegion = function (addressType) {
        if (addressType === shippingAddressType) {
            var countryRegion = _.find($scope.checkout.shippingCountryRegions, function (r) {
                return r.name === $scope.checkout.shipment.deliveryAddress.regionName;
            });
            if (countryRegion) {
        ...
        if (addressType === billingAddressType) {
            var countryRegion = _.find($scope.checkout.billingCountryRegions, function (r) {
                return r.name === $scope.checkout.payment.billingAddress.regionName;
            });
            if (countryRegion) {
    ...
    $scope.selectCustomerAddress = function (addressType) {
        console.log($scope.checkout.selectedCustomerAddress.CountryCode);
        //var country = _.find($scope.checkout.countries, function (c) {
        //    return c.code3 === customerAddress.countryCode;
        //});
        //if (addressType === shippingAddressType) {
        //    $scope.checkout.shipment.deliveryAddress = customerAddress;
        //    $scope.checkout.shipment.deliveryAddress.type = shippingAddressType;
        //    $scope.checkout.selectedShippingCountry = country;
        //    if (customerAddress.countryCode) {
        //        $scope.checkout.selectedShippingCountry = country;
        //        getCountryRegions(shippingAddressType, customerAddress.countryCode);
        //    }
        //}
        //if (addressType === billingAddressType) {
        //    $scope.checkout.payment.billingAddress = customerAddress;
        //    $scope.checkout.payment.billingAddress.type = billingAddressType;
        //    if (customerAddress.countryCode) {
        //        $scope.checkout.selectedBillingCountry = country;
        //        getCountryRegions(billingAddressType, customerAddress.countryCode);
        //    }
        //}
    }
    ...
    function initialize() {
        ...
        getCountries();
        getCart();
        getCountries(shippingAddressType);
    }
    ...
    function getCustomer() {
        customerService.getCurrentCustomer().then(function (response) {
            ...
            _.each(customer.addresses, function (address) {
                address.stringifiedAddress = $scope.stringifyAddress(address);
            });
    ...
    function getCart() {
        cartService.getCart().then(function (response) {
            ...
            var payment = cart.payments.length ? cart.payments[0] : {};
            payment.billingAddress = payment.billingAddress || cart.defaultBillingAddress;
            if (payment.billingAddress.countryCode) {
                $scope.checkout.selectedBillingCountry = _.find($scope.checkout.countries, function (c) {
                    return c.code === payment.billingAddress.countryCode;
                });
                getCountryRegions(billingAddressType, payment.billingAddress.countryCode);
            }
            var shipment = null;
            if (cart.hasPhysicalProducts) {
                shipment = cart.shipments.length ? cart.shipments[0] : {};
                shipment.deliveryAddress = shipment.deliveryAddress || cart.defaultShippingAddress;
                if (shipment.deliveryAddress.countryCode) {
                    $scope.checkout.selectedShippingCountry = _.find($scope.checkout.countries, function (c) {
                        return c.code3 === shipment.deliveryAddress.countryCode;
                    });
                    getCountryRegions(shippingAddressType, shipment.deliveryAddress.countryCode);
                }
            }
    ...
    function getCountryRegions(addressType, countryCode) {
        cartService.getCountryRegions(countryCode).then(function (response) {
            ...
            if (addressType === shippingAddressType) {
                $scope.checkout.shippingCountryRegions = countryRegions || [];
            }
            if (addressType === billingAddressType) {
                $scope.checkout.billingCountryRegions = countryRegions || [];
            }
        });
    }
    ...
    function compareObjects(obj1, obj2, excludedFields) {
        _.each(excludedFields, function (excludedField) {
            if (obj1 && obj2) {
                obj1.excludedField = null;
                obj2.excludedField = null;
            }
        });
