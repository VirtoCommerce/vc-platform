...
{
    /// Implementation of https://github.com/Shopify/liquid/blob/master/performance/shopify/paginate.rb
    /// <summary>
    ...
    public class Paginate : Block
    {
        ...
        {
            if (context.Registers["path"] != null)
                CurrentUri = context.Registers["path"] as Uri;
            else
                CurrentUri = new Uri("/");
            ...
            var uri = context.Registers["path"] as Uri;

            var previousLink = uri.SetQueryParameter("page", (this._currentPage - 1).ToString());
            var nextLink = uri.SetQueryParameter("page", (this._currentPage + 1).ToString());
            ...
            context["paginate"] =
                Hash.FromAnonymousObject(
                    new
                    {
                        ...
                        previous =
                            (this._currentPage > 1)
                                ? new Part() { IsLink = true, Title = "&laquo; Previous", Url = previousLink.ToString() }
                                : null,
                        next =
                            (this._currentPage < pageCount)
                                ? new Part() { IsLink = true, Title = "Next &raquo;", Url = nextLink.ToString() }
                                : null,
                        size = length,
                        current_offset = (this._currentPage - 1) * this._pageSize,
                        current_page = this._currentPage,
            ...
            while (index < pageCount)
            {
                parts.Add(
                    new Part()
                    {
                        ...
                        Url = uri.SetQueryParameter("page", (index + 1).ToString()).ToString()
                    });
                index++;
            }
